...
set.seed(12345)
df <- data.frame(CATEGORY,PRICE, CONTENT_RATING,DOWNLOAD_MIN,MIN_REQ_ANDROID_FIRST,TOTAL_REVIEWS,AVERAGE_RATING) #creating a dataset with limited fields from the ones we read as csv

data <- df
num.vars <- sapply(data, is.numeric) #converting all fields to numeric
data[num.vars] <- lapply(data[num.vars], scale) #scale the fields
df <- data

df_new <- dummy.data.frame(df, names = c("CATEGORY","CONTENT_RATING","MIN_REQ_ANDROID_FIRST"), sep = ".") #creating dummy variables
df_new <- cbind(df_new, spam) # putting the predictor variable in the end
ggcorr(DM) #find correlation amongst variables

############################Creating Training and Testing Datasets####################################

train_ind<-sample(nrow(df_new),0.7*nrow(df_new))
train <- df_new[train_ind,]  #Training dataset
test <- df_new[-train_ind,] #testing dataset
nrow(train)
nrow(test)
...
###########Predicition on the training dataset###################
predict_train <- predict(bfit, newdata = train[,1:45])
t3 <- ifelse(predict_train > 0.5,1,0)  #setting the cutoff value as 0.5 for predicting whether the app is spam or not
t3
Confusion_train <- table(as.numeric(train$spam),t3)
Confusion_train         #plotting the confusion matrix
actual <- as.numeric(train$spam)
Metrics <- c("AE","RMSE","MAE") #determing the Average error, Root Mean Square error and Mean average error
...
###########Predicition on the test dataset###################
...
train_under <- rbind(train_under, rearrangedZero_spams) #Training data set using undersampling
